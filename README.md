...
[![Версия 1.3.0](https://img.shields.io/badge/version-1.3.0-173F46)](project.json) [![Формат Markdown](https://img.shields.io/badge/format-Markdown-53616B)](skill/prompt-improver/SKILL.md) [![Аудит на 50 темах](https://img.shields.io/badge/evaluation-50%20topics-173F46)](evals/AUDIT-50-2026-09-06.md)
...
### Задать правку существующего материала

Уточните, что изменить в тексте, изображении, коде или таблице и какие свойства сохранить. Скилл подготовит задание с исходником, границами правки и способом сравнить результат. Недоступные материалы останутся явным условием передачи.

### Подготовить поручение агенту

Для работы с файлами и инструментами скилл связывает исходное состояние, конечный результат, разрешённые действия и фактическую проверку выполнения. Уже данные разрешения сохраняются в своих границах.

### Описать автоматизацию

Скилл помогает определить событие запуска, данные, действия и ожидаемый эффект. По необходимости уточняет повторы, пропущенные поля и восстановление после частичного сбоя. Схема процесса и действующее подключение различаются явно.

Подробнее об этих сценариях и сохранении договорённостей — в [описании возможностей 1.3.0](docs/FEATURES-1.3.0.md).

### Исправить промпт по неудачному ответу

Передайте исходный запрос, полученный ответ и то, что ожидали. Скилл разберёт расхождение, сохранит рабочие требования и подготовит исправленную версию. Если проблема в отсутствии данных или инструментов, это останется явно обозначенным.

```text
$prompt-improver исправь мой промпт. Мне нужен JSON с числом или null,
а модель пишет пояснение и заменяет неизвестное нулём.
Исходный промпт: [...]. Неудачный ответ: [...]. Без вопросов.
```

### Подготовить шаблон для повторных задач

Скилл отделит постоянные правила от новых входных данных, опишет переменные, пропуски и формат ответа. По необходимости добавит примеры и отдельные проверочные случаи.

```text
$prompt-improver подготовь шаблон для ежедневной классификации обращений.
Категории: доставка, возврат, другое. Если подходят две категории — уточнение.
Вход — текст обращения. Выход — JSON с полями category и reason.
Без вопросов, дай шаблон и два учебных примера.
```

### Уточнить задачу в предметной области
...
|---|---|
| [Проверка 1.3.0](evals/UPGRADE-1.3.0.md) | Девять случаев на двух версиях и промежуточной редакции, брифинги и исполнения; 42 вызова модели, исходные ответы и ограничения проверки |
| [Аудит на 50 темах, версия 1.2.0](evals/AUDIT-50-2026-09-06.md) | 50 диалогов, 50 промптов и 50 исполнений; два отдельных оценивания каждого случая, расчёты и полный разбор замечаний |
| [Обновление 1.2.0](evals/UPGRADE-1.2.0.md) | Восемь новых сценариев и четыре исполнения готовых промптов; входы, ответы и проверка JSON опубликованы |
...
- [Подробный разбор Prompt Master](docs/PROMPT-MASTER-REVIEW-2026-09-06.md) — 13 шаблонов, 37 паттернов, проверка спорных рекомендаций и план применения у нас.
